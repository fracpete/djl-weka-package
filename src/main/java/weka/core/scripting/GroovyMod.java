 ...
 */
public class GroovyMod implements Serializable, RevisionHandler {
  ...
  public static Object newClassLoader() {
    return newClassLoader(null);
  }

  /**
   * initializes and returns a Groovy Interpreter.
   *
   * @param classloader the class loader to use, determines one if null
   * @return the interpreter or null if Groovy classes not present
   */
  public static Object newClassLoader(ClassLoader classloader) {
    Object result;
    Class<?> cls;
    Constructor<?> constr;
	...
	constr = cls.getConstructor(ClassLoader.class);
	result = constr.newInstance(classloader);
      } catch (Exception e) {
	e.printStackTrace();
	result = null;
      }
  ...
  public static Object newInstance(File file, Class<?> template) {
    return newInstance(file, template, null);
  }
   ...
   * @param file the Groovy module file
   * @param template the template for the returned Java object
   * @param classLoader the class loader to use, determines one automatically if null
   * @return the Groovy object
   */
  public static Object newInstance(File file, Class<?> template, ClassLoader classLoader) {
    Object result;
    Object interpreter;
    Class<?> cls;
    ...
    interpreter = newClassLoader(classLoader);
    if (interpreter == null) {
      return result;
    }
  ...
  public static void main(String[] args) {
    if (args.length == 0) {
      ...
      GroovyMod groovy = new GroovyMod();
      if (groovy.getClassLoader() == null) {
	System.err.println("Cannot instantiate Groovy ClassLoader!");
      } else {
	Object groovyObject = GroovyMod.newInstance(new File(args[0]),
	  Object.class);
	GroovyMod.invoke(groovyObject, "run", new Class[] {}, new Object[] {});
      }
    }
  }
}
